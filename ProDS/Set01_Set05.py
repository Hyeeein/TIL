...
# ols
# 변수1 = ols(식, 데이터).fit() -> 이 방법이 가장 편함 / 식 : 'y~x1+x2+x3-1', -1의 의미 : 상수항 미포함, c() : 범주형
# 변수1 = ols(식, 데이터)
# 변수2 = 변수1.fit()
...
q2_out = q2_out[q2_out.pvalue < 0.05]
len(q2_out2)
...
# 1. 한국/일본 필터링
q2 = data4[data4.LOATION.isin(['KOR', 'JPN'])]
...
for i in sub_list:
    ...
    temp2 = pd.pivot_table(temp, index='TIME', columns='LOCATION', value='Value', aggfunc='mean').dropna()
...
q3 = q3[q2.Segmentation.isin(['A', 'D'])]

# 2. Train 대 Test 7대 3으로 데이터를 분리 (Seed = 123)
from sklearn.model_selection import train_test_split

train, test-train_test_split(q3, test_size=0.3, random_state=123)

# 3. Train 데이터를 사용하여 의사결정나무 학습을 수행하고, Test 데이터를 병행
